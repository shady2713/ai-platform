...
| `AiReportController#refresh` | 同上：只有可刷新（REFRESHABLE）报表可刷新；按当前权限重放版本里的固定查询版本（不重新规划、不调模型），全部成功才用乐观锁新增版本并切换生效版本；失败保留旧结果并留痕（尝试记录带时间与稳定原因），停用/漂移/失权即停（AT-047/048） |
| `AiReportController#refreshLast` | 同上：读取「上次刷新时间/结果/原因 + 上一次结果数据」；上一次结果在返回前逐项复核 A03 范围指纹，失权即拒绝（不为了页面还能看而跳过再鉴权） |
