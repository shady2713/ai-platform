...
// FormRenderProps.form 类型上可选，但 form-field 必须由 form 渲染器提供实例
function requireFormApi(): FormActions {
  if (!formApi) {
    throw new Error(
      'FormField must be rendered inside a form with a form instance',
    );
  }
  return formApi;
}
...
const computedProps = computed(() => {
  const finalCommonComponentProps = isFunction(commonComponentProps)
    ? commonComponentProps(values.value, requireFormApi())
    : commonComponentProps;
  const finalComponentProps = isFunction(componentProps)
    ? componentProps(values.value, requireFormApi())
    : componentProps;
...
const customContentRender = computed(() => {
  ...
  return renderComponentContent(values.value, requireFormApi());
});
