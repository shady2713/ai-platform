...
## 用户类型 Provider 扩展（A05）

每个用户类型必须且只能有一个 `UserSessionCommonApi` 实现，容器启动时索引：

- 声明未知用户类型、声明为空、同一类型重复实现都会在启动期抛 `IllegalStateException`；
- 请求未声明用户类型且存在多个 Provider 时，认证链路直接拒绝（`AccessDeniedException`），
  **不按任意 Provider 兜底**——避免把管理员会话与面向 C 端的会员会话互相降级；
- 请求的用户类型与 Provider 返回的会话类型不一致时拒绝（"错误的用户类型"）。

AI 中台的 MEMBER 会话由 `module-ai` 的 `AiUserSessionCommonApi` 提供：票据（A04）在校验时重新读取
应用与主体状态，撤销/到期/停用立即失效；会话只携带服务端建立的字段（应用、主体类型、可信外部用户标识、
范围指纹），外部主体身份不映射为系统用户编号。

## AI 应用端 scope 表达式（A05）

应用端（`/app-api/ai/**`）使用 `@PreAuthorize("@aiScope.hasScope('REPORT','report-1','READ')")`：
判定委托 AI 中台的统一授权入口（应用 + 主体范围 + 授权目录 + 动作白名单），表达式自身不缓存；
没有登录用户、缺少 AI 会话信息、资源类型/动作不在目录内或判定异常时**一律拒绝**（fail-closed）。
资源类型、动作与匿名端点清单见 `docs/contracts/ai/scope-catalog.md`。
